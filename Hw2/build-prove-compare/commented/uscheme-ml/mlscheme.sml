...
(* Because error-handling is so much easier in ML than *)
(* in C, we provide two forms of unit test.     *)
(* [[CHECK_EXPECT]] is the same form used in earlier *)
(* chapters; [[CHECK_ERROR]] asserts that evaluating an *)
(* expression should signal an error.           *)
(* <abstract syntax and values ((mlscheme))>=   *)
datatype unit_test = CHECK_EXPECT of exp * exp
                   | CHECK_ERROR  of exp
(* Definitions are straightforward, but for the ML code *)
(* in this book, I've split them into two parts. A  *)
(* [[def]], for ``definition,'' is a definition like *)
(* [[VAL]], [[EXP]], or [[DEFINE]], which is actually *)
(* part of the language. An [[xdef]], is an ``extended *)
(* definition,'' which includes all of the [[def]] *)
(* forms, plus [[USE]] and the unit tests. Why split *)
(* definitions in this way? Because the code for *)
(* handling unit tests and [[USE]] is generic, and *)
(* splitting these forms away from the others makes this *)
(* generic code easier to reuse.                *)
(* <abstract syntax and values ((mlscheme))>=   *)
...
(* are compared for equality structurally.      *)
...
(* Function [[dfn]] is a bit like [[lambda]]: it detects *)
(* duplicate formal parameters. The name ``[[dfn]]'' *)
(* allows me more horizontal space than I would have if *)
(* I used ``[[define]].''                       *)
(* <boxed values 81>=                           *)
...
(* Function [[schemeReader]] uses the lexer and parser *)
(* to make a function that converts a stream of lines to *)
(* a stream of extended definitions.            *)
(* <boxed values 84>=                           *)
...
(* When we see a [[val]], we add the name to the *)
(* environment, evaluate the right-hand side, and print *)
(* the result of [[showVal]], which is either the value *)
(* of the right-hand side or, in the case of a binding *)
(* to a function, the name of the function. As usual, *)
(* [[define]] is syntactic sugar. When we get an *)
(* expression, we print its value and bind the result *)
(* to [[it]].                                   *)
(* <boxed values 10>=                           *)
...
(* <unit testing ((mlscheme))>=                 *)
val testEqual = equalpairs
(* <unit-testing utilities>=                    *)
(* <definition of function [[expString]] ((mlscheme))>= *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
...
(* <unit-testing utilities ((mlscheme))>=       *)
fun testsPassed (tests, rho) =
  let fun fail strings = (app eprint strings; eprint "\n"; false)
      ...
      (* <unit-testing utilities that depend on [[outcome]]>= *)
      ...
      (* <definition of function [[passes]] ((mlscheme))>= *)
      fun passes (CHECK_EXPECT (c, e)) = checkExpectPasses (c, e)
        | passes (CHECK_ERROR c)       = checkErrorPasses  c
(* Unit testing                                 *)
(*                                              *)
(* When running a unit test, we have to account for the *)
(* possibility that evaluating an expression causes a *)
(* run-time error. Just as in Chapters [->] and [->], *)
(* such an error shouldn't result in an error message; *)
(* it should just cause the test to fail. (Or if the *)
(* test expects an error, it should cause the test to *)
(* succeed.) To manage errors in C, we had to fool *)
(* around with [[set_error_mode]]. In ML, things are *)
(* simpler: we convert the result of evaluation either *)
(* to [[OK]] v, where v is a value, or to [[ERROR]] m, *)
(* where m is an error message. The [[OK]] and [[ERROR]] *)
(* constructors are defined in \appreflazyparse, where *)
(* they are used to represent results of parsing input. *)
(* <boxed values 85>=                           *)
...
(* operators, then adding the initial basis. When *)
(* reading the initial basis, the interpreter issues no *)
(* prompts, makes no responses, and even ignores error *)
(* messages.                                    *)
