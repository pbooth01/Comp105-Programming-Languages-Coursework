...
(* [[TextIO]] can be a bit awkward, so here, I add three *)
(* convenience functions: [[println]] is like [[print]], *)
(* but writes a string followed by a newline. And *)
(* functions [[eprint]] and [[eprintln]] are analogous *)
(* to [[print]] and [[println]], except that they write *)
(* to standard error. It would be nice to be able to *)
(* define more sophisticated printing functions like the *)
(* ones in \secrefsec:print-interface on page [->], but *)
(* making such functions type-safe requires code that *)
(* beginning ML programmers would find a bit baffling. *)
(* <convenience functions for writing to standard output and standard error>= *)
fun println  s = (print s; print "\n")
fun eprint   s = TextIO.output (TextIO.stdErr, s)
fun eprintln s = (eprint s; eprint "\n")
(* To help you diagnose problems in your code, I also *)
(* provide a function for reporting errors that are *)
(* detected while reading an initial basis.     *)
(* <convenience functions for writing to standard output and standard error>= *)
...
(* ignoring errors. When I use [[peek]], it's usually to *)
(* peek at the source-code location of the next token in *)
(* a token stream.                              *)
(* <boxed values 77>=                           *)
...
(* Transformer [[oneEq x]] is an \atoa transformer that *)
(* succeeds if and only if there is a next input and *)
(* that input equals [[x]]. It is typically used to *)
(* recognize special characters like parentheses and *)
(* minus signs.                                 *)
(* <boxed values 80>=                           *)
...
(* input—it consumes only the input of [[t2]]. It's *)
(* usually used to detect errors, as in ``when you see *)
(* something bad, signal an error.''            *)
(* <boxed values 82>=                           *)
val _ = op <&> : ('a, 'b) xformer * ('a, 'c) xformer -> ('a, 'c) xformer
(* <parsing combinators>=                       *)
...
(* a condition. Parser [[notFollowedBy t]] succeeds if *)
(* and only if [[t]] fails. Parser [[notFollowedBy t]] *)
(* may look at the input, but it never consumes any *)
(* input. I use [[notFollowedBy]] when reading integer *)
(* literals, to make sure that the digits are not *)
...
(* [[int option]] to [[int]]. The [[Int. ]] function, *)
(* which is used when we see a minus sign, negates an *)
(* integer. The [[ ]] is meant to resemble a ``high *)
(* minus'' sign.                                *)
(* <boxed values 92>=                           *)
...
(* returns [[OK]] applied to the list of names; *)
...
(* ( ***************************************************************** ) *)
(*                                              *)
(* ]]. In the ML code, I can do the job in      *)
(* a very modular way: I define a post-stream action *)
(* that prints any line meeting the criterion. Function *)
(* [[echoTagStream]] transforms a stream of lines to a *)
(* stream of lines, adding the behavior I want. *)
(* <boxed values 105>=                          *)
...
and method_impl = USER_IMPL of name list * name list * exp
                | PRIM_IMPL of name
(* Extended definitions and unit tests are as in *)
(* micro-Scheme.                                *)
(* <abstract syntax and values ((usm))>=        *)
and xdef      = DEF  of def
              | USE  of name
...
fun exp tokens = (
  ...
  <|> SUPER              <$  literal "super"
  <|> VAR                <$> name
  <|> literal "#" *> (   LITERAL <$> SYM                  <$> name
                    <|>  LITERAL <$> SYM <$> Int.toString <$> int
                    <|>  VALUE                            <$> sharp
...
(* <parsing ((usm))>=                           *)
val smalltalkReader = reader (smalltalkToken, xdef)
...
(* <generic read-eval-print loop>=              *)
(* <type [[echo]] and function [[echoes]], for controlling whether definitions are echoed>= *)
...
(* [[echo]], which is either [[ECHOING]] or [[SILENT]]. *)
(* When [[ECHOING]], after each definition the  *)
(* interpreter writes a response to standard output. *)
(* When [[SILENT]], it doesn't.                 *)
(* <boxed values 29>=                           *)
...
(* <definition of [[processXDef]], which can modify [[unitTests]] and call [[errmsg]]>= *)
            ...
            (* It is [[processXDef]] that does the bulk of the work. *)
            (* The main job of [[processXDef]] is to implement the *)
            (* extended-definition forms [[USE]] and [[TEST]], which *)
            (* it does in exactly the same way for every    *)
            (* interpreter: [[USE]] is implemented by function *)
            (* [[useFile]], which reads definitions from a file and *)
            (* recursively passes them to [[processXDefs]]. The *)
            (* [[read]] function, which knows how to convert strings *)
            (* to definitions, is an argument to [[readEvalPrint]]. *)
            (* Each [[TEST]] is simply accumulated into the mutable *)
            (* list [[unitTests]], just as in the C code in \ *)
            (* secrefpageimpcore.readevalprint. The most interesting *)
            (* job—processing true definitions—delegated to *)
            (* [[processDef]], another function that is passed as an *)
            (* argument to [[readEvalPrint]].               *)
            (* <boxed values 32>=                           *)
            ...
            val _ = op processDef : def  * 'envs -> 'envs
              in  (case xd
                    ...
                    (* New exception handlers for the generic       *)
                    ...
                    | Size            => continue
                              "asked for array of negative size (or really big)"
                    (* The final job of [[processXDef]] is to deal with *)
                    (* errors. The [[case]] expression in the body is *)
                    (* protected by a collection of exception handlers, each *)
                    (* of which calls [[continue]] to print an error message *)
                    (* and to return the (unchanged) environments [[envs]]. *)
                    (* We reuse the same exception handlers in later *)
                    (* interpreters. The first handler takes care of *)
                    (* exception [[IO.Io]], which is part of the Standard *)
                    (* Basis Library. The remaining handlers deal with *)
                    (* problems that arise during evaluation.       *)
...
(* The looping action comes from function       *)
(* [[streamFold]], which applies [[processXDef]] to *)
(* every element of [[xdefs]]. Function [[streamFold]] *)
(* is the stream analog of the list function [[foldl]]. *)
...
(* <private state and functions for mainting a stack of source-code locations ((usm))>= *)
      ...
      (* <function [[ev]], the evaluator proper ((usm))>= *)
        | ev (SEND (srcloc, message, receiver, args))  =
            let val obj as (class, rep) = ev receiver
                ...
                (* ((usm))>>]]. Chunk [[]] also                 *)
                (* defines function [[eprintlnTrace]], which is called *)
                (* from chunks [->] and [->] to show the stack of active *)
                (* message sends after a run-time error.        *)
  ...
  case !(find (supername, xi))
    of (meta, CLASSREP c) => (meta, c)
     | v => raise RuntimeError ("object " ^ supername ^ " = " ^ valueString v ^
                                " is not a class")
(* <evaluation ((usm))>=                        *)
fun evaldef echo (d, xi) =
  let val _ = closeLiteralsCycle xi handle NotFound _ => ()
      fun ev e = eval (e, emptyEnv, objectClass, xi)
      (* Function [[doVal]] is optimized so that if a previous *)
      (* binding exists, we replace the binding and do not *)
      (* create a new global environment. The optimization is *)
      (* safe only because no operation in uSmalltalk makes a *)
      (* copy of the global environment.              *)
      (* <definition of [[doVal]]>=                   *)
      fun doVal (name, value) =
        ( (find (name, xi) := value; xi)
           handle NotFound _ => bind (name, ref value, xi)
        )
        before (* Unlike our other interpreters, our uSmalltalk *)
               (* interpreter doesn't need special-case code to print *)
               (* top-level expressions differently from [[val]] *)
               (* bindings; every value is printed by sending it the *)
               (* [[println]] message. The default [[println]] method *)
               (* calls the object's [[print]] method, which you can *)
               (* redefine.                                    *)
               (* <if called for by [[echo]], send [[println]] to [[value]]>= *)
               ( if echoes echo then 
                   ignore (ev (SEND (nullsrc, "println", VALUE value, [])))
                 else
                   ()
               )
...
(* environment. But in uSmalltalk, it also has a side *)
(* effect on the state of the interpreter. The  *)
(* implementation is best understood in four steps: *)
(*                                              *)
(*  1. First, call [[closeLiteralsCycle]]. This call *)
(*  updates the definitions (if any) of         *)
(*  [[SmallInteger]], [[Symbol]], and [[Array]]. *)
(*  By updating these definitions, we make it   *)
(*  possible for you to change the behavior of these *)
(*  classes (as is required in Exercises [->], [->], *)
(*  [->], and [->]).                            *)
(*  2. Define auxiliary function [[ev]], which evaluates *)
(*  an expression [[e]] as if in the context of a *)
(*  message sent to a value of class [[Object]]. That *)
(*  means using an empty rho, using [[Object]] as the *)
(*  superclass, and using the given [[xi]].     *)
(*  3. Define auxiliary function [[doVal]], which adds a *)
(*  binding to the environment [[xi]]. Function *)
(*  [[doVal]] also echoes the value defined,    *)
(*  if called for by [[echo]]. Because in Smalltalk, *)
(*  everything is a value, even a new class, all *)
(*  definitions cause bindings to be added by   *)
(*  [[doVal]].                                  *)
(*  4. Finally, analyze the definition [[d]], and return *)
(*  the new xi with the binding for that definition. *)
(*                                              *)
(* Here's the code:                             *)
(* <boxed values 19>=                           *)
val _ = op evaldef : echo -> def * value ref env -> value ref env
val _ = op doVal   : name * value -> value ref env
val _ = op ev      : exp -> value
  in  case d
        of VAL (name, e)             => doVal (name, ev e)
         | EXP e                     => doVal ("it", ev e)
         | DEFINE (name, args, body) => doVal (name, ev (BLOCK (args, [body])))
         | CLASSD (d as {name, ...}) => doVal (name, newClassObject d xi)
   end
(* The [[VAL]] and [[EXP]] forms both evaluate the given *)
(* expression and bind it to a name—either the given *)
(* [[name]] or [["it"]]. [[DEFINE]] is syntactic sugar *)
(* for a definition of a block. And evaluating a class *)
(* definition binds a new class object, as described *)
(* above.                                       *)
...
(* Unit testing                                 *)
(*                                              *)
(* A little different, because equality requires *)
(* evaluation in the interpreter.               *)
(* <unit testing ((usm))>=                      *)
fun runTests (tests, xi) =
  let fun ev e = eval (e, emptyEnv, objectClass, xi)
      fun outcome e = OK (ev e) handle _ => ERROR "evaluation failed"
      fun testEqual (v1, v2) =
      ...
      fun fail strings = (app eprint strings; eprint "\n"; false)
      fun valueString _ =
        raise RuntimeError "internal error: called the wrong ValueString"
      (* <definition of function [[expString]] ((usm))>= *)
      fun expString e =
        let fun bracket s = "(" ^ s ^ ")"
            val bracketSpace = bracket o spaceSep
            fun exps es = map expString es
            fun numString n = String.map (fn #"~" => #"-" | c => c) (
                                                                 Int.toString n)
            fun symString x = x
            fun valueString (_, NUM n) = numString n
              | valueString (_, SYM x) = symString x
              | valueString (c, _) = "<" ^ className c ^ ">"
        in  case e
              of LITERAL (NUM n) => numString n
      ...
      fun checkExpectPasses (checkx, expectx) =
        case (outcome checkx, outcome expectx)
          of (OK check, OK expect) => 
           ...
           | (ERROR _, tried) =>
               fail [cxfailed, "evaluating ", expString checkx,
                                                            " caused an error."]
           | (_, ERROR _) =>
               fail  [cxfailed, "evaluating ", expString expectx,
                                                            " caused an error."]
      (* <unit-testing utilities ((usm))>=            *)
      ...
      fun checkErrorPasses checkx =
            case outcome checkx
              of ERROR _ => true
               | OK check =>
                   fail ["check-error failed; diagnostic on standard output"]
                   before
      ...
      (* <unit-testing utilities ((usm))>=            *)
      fun testsPassed (tests, xi) =
        let (* <definition of function [[passes]] ((usm))>= *)
  ...
  in  reportTestResults (testsPassed (tests, xi), length tests)
  end
...
(* set [[echoBasis]] to [[true]]. Also to help  *)
(* debugging, the interpreter uses an anonymous function *)
(* to report errors. This function identifies the error *)
(* as originating in the initial basis, and it uses *)
(* [[eprintlnTrace]] instead of [[eprintln]], so that if *)
(* an error occurs, a stack trace is printed. [*] *)
(* <initialization ((usm))>=                    *)
val echoBasis = SILENT
val initialXi =
  let val defs = smalltalkReader noPrompts
                 ("initial basis", streamOfList
                              (* Further reading                              *)
                           ...
                           (* \citetwirth:unnecessary-diversity master of the *)
                              ...
                              (* [cite mcbride-paterson:applicative]          *)
...
"      (remove:ifAbsent: self oldObject [(error: self #tried-to-remove-absent-object)]))"
   ...
   "    (class-method new ()        (sentinel: (new super) (new ListSentinel)))"
    ...
    "    (method collect: (_) (error: self #collect-on-arrays-not-implemented))"
      ...
      fun errmsg s = eprintlnTrace ("error in initial basis: " ^ s)
      val repl =
        readEvalPrint runTests (smalltalkReader noPrompts) basisError (evaldef
                                                                      echoBasis)
  in  repl (defs, initialXi)
  end
...
in
  val initialXi = evaldef SILENT (VAL ("true",  newInstance "True" ), initialXi)
  val initialXi = evaldef SILENT (VAL ("false", newInstance "False"), initialXi)
end
...
(* <initialization ((usm))>=                    *)
fun addVal ((name, v), xi) = evaldef SILENT (VAL (name, VALUE v), xi)
...
fun runInterpreter prompts = 
  let val defs = smalltalkReader prompts ("standard input", streamOfLines
                                                                   TextIO.stdIn)
      val repl = readEvalPrint runTests (smalltalkReader noPrompts)
                                                                   eprintlnTrace
                 (evaldef ECHOING) 
  in  ignore (repl (defs, initialXi))
  end
(* The function [[runInterpreter]] takes one argument, *)
(* which tells it what prompts to use, if any. Again, we *)
(* use [[eprintlnTrace]] instead of [[eprintln]], so *)
(* that if a run-time error occurs, the interpreter will *)
(* print a stack trace. [*]                     *)
(* <boxed values 20>=                           *)
