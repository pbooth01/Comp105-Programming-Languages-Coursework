...
(* one I've chosen for the representation of a  *)
(* substitution is a finite map from type variables to *)
(* types. In our code, we have a standard representation *)
(* for such a map: an environment of type [[ty env]]. *)
...
(* the [[freshtyvar]] function. We use a private mutable *)
(* counter to supply an arbitrary number of type *)
(* variables of the form t n. Because \nml does not *)
(* permit users to write type variables explicitly, we *)
(* need not worry about the names colliding with other *)
(* names.                                       *)
(* <boxed values 10>=                           *)
...
infix 4 ~
infix 3 /\
(* (If you have programmed in ML before, you may be *)
(* accustomed to use [[ ]] as the unary minus operator. *)
(* Our code redefines [[ ]], but the original version *)
(* can still be used by its qualified name [[Int. ]].) *)
...
(* \nml by presenting type definitions from our *)
...
and (* At a mathematical level, \nml and micro-Scheme have *)
    ...
    (* a cycle. In our implementation of \nml, we build each *)
...
(* \nml is an impure language because it has two *)
...
datatype xdef = DEF    of def
              | USE    of name
              | TEST   of unit_test
(* Unit tests are as in Typed uScheme.          *)
(* <abstract syntax and values ((ml))>=         *)
...
(* Parsing                                      *)
(*                                              *)
(* \nml can use micro-Scheme's lexical analysis, so all *)
...
(* <function [[typesof]], to infer the types of a list of expressions ((ml/haskell))>= *)
...
(* <function [[ty]], to infer the type of an expression, given [[Gamma]] ((ml))>= *)
    ...
    of VAL    (x, e)      =>
     ...
     | VALREC (x, e)      =>
                     ...
                     (* threeline \typeisc[{x |->alpha}] \tyce tau\qquadalpha *)
                      ...
                      (* \ftv(Gamma) \topt\xvalrec(x, e) -->Gamma{x |->sigma} *)
                             (*                                              *)
...
(* comparisons apply only to integers. We get to reuse *)
...
(* <definition of function [[expString]] ((ml))>= *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
  ...
  let fun fail strings = (app eprint strings; eprint "\n"; false)
      fun ty e = typeof (e, Gamma)
                 handle NotFound x => raise TypeError ("name " ^ x ^
                                                              " is not defined")
      (* <unit-testing utilities that depend on [[ty]] and [[Gamma]]>= *)
      ...
      (* <definition of function [[checks]] ((ml))>=  *)
      ...
      (* <definition of function [[passes]] ((ml))>=  *)
      ...
      val envs  = foldl addPrim (emptyTypeEnv, emptyEnv) (
                     ...
                     _   => raise BugInTypeInference "cdr applied to non-list"),
                      ...
                      (* The last two primitives are [[print]] and [[error]]. *)
                        ...
                        (* The type of [[error]], \/alpha,beta\alldotalpha--> *)

                            (* beta, tells us something interesting about its *)

                            (* behavior. The type suggests that [[error]] can *)
