 ...
 *  also be set.)
 * 
 * [*] For ease of garbage collection later, the stack
...
Value eval(Exp e, Env env) {
    ...
    exp: 
         ...
         * 20 expression forms in \uschemeplus are legitimate.
        ...
        case LETX:
            if (/*
                ...
                switch (e->u.letx.let) {
                   case LET:     /*
                                  * Evaluating a \xlet expression pushes a
                                                                      context in
                                  * which the first expression is replaced by a
                                                                        hole. We
                                  * build the context using [[copyEL]] and
                                                                      insert the
                                  * hole using function
                                                    [[head_replaced_with_hole]].
                                  * <start evaluating nonempty [[let]]
                     expression [[e->u.letx]] and transition to the next state>=
                                  */
                                 /*
                                  * <if [[e->u.letx.nl]] contains a duplicate,
                                                  complain of error in [[let]]>=
                                  */
                                 ...
                                 goto exp;
                   case LETSTAR: /*
                                  * Implementing \xletstar is simpler; there are
                                                                              no
                                  * holes. Because there are no holes and
                                                                 nothing is ever
                     ...
                     expression [[e->u.letx]] and transition to the next state>=
                                 ...
                                 goto exp;
                   case LETREC:  /*
                                  * Evaluating a \xletrec expression is very
                                                                        similar.
                                  * We first save the environment on the stack
                                                                      and extend
                                  * it with locations containing unspecified
                                                                    values. Then
                                  * we do what we did for \xlet: replace the
                                                                           first
                                  * expression with a hole and evaluate that
                                                                     expression.
                                  * <start evaluating nonempty [[letrec]]
                     expression [[e->u.letx]] and transition to the next state>=
                                  */
                                 /*
                                  * <if [[e->u.letx.nl]] contains a duplicate,
                                               complain of error in [[letrec]]>=
                                  */
                                 if (duplicatename(e->u.letx.nl) != NULL)
                                     error(
    ...
    value: 
        ...
        if (fr == NULL) {
            ...
            case APPLY:
                 ...
                 * there are three rules that might apply: If rule \
                 * rulenameSmall-Step-Apply-First-Arg applies, the
                 * expression [[fr->context.u.apply.fn]] in function
                 * position will be a hole. If rule \rulename
                 * Small-Step-Apply-Next-Arg applies, function
                 * [[transition_explist]] will return the next argument.
                 * If rule \rulenameSmall-Step-Apply-Last-Arg applies,
                 * function [[transition_explist]] will return NULL.
                 * There is also a case not given in the semantics, when
                ...
                apply_last_arg:
                     ...
                     */
                    {
                        ...
                        switch (fn.alt) {
                          case PRIMITIVE:
                              /*
                               * Primitives are also applied in the standard
                                                                            way.
                               * <apply [[fn.u.primitive]] to [[vl]] and
                          ...
                          case CLOSURE:
                               ...
                               * body of the closure, we save the environment on
                                                                             the
                               * stack.
                               * <save [[env]], bind [[vl]] to [[fn.u.closure]]
            ...
            case LETX:
                switch (fr->context.u.letx.let) {
                   case LET:     /*
                                  ...
                                  * progress: Function [[transition_explist]]
                                                                       puts v in
                   ...
                   case LETSTAR: /*
                                  ...
                                  * position. We don't even have to look for it:
                                                                        Name x_i
                                  * is at the head of [[fr->context.u.letx.nl]].
                                                                         We bind
                                  * it and move on.
                                  * <continue with [[let*]] context
                                                        [[fr->context.u.letx]]>=
                   ...
                   case LETREC:  /*
                                 ...
                                 if (e) {  /* Small-Step-Next-Letrec-Exp */
                                     ...
                                     /*
                                      * <store values in
     [[fr->context.u.letx.el]] in locations bound to [[fr->context.u.letx.nl]]>=
                                      */
                                     {
