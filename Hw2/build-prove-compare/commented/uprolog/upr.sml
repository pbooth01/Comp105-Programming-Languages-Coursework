...
(* enables us to use the identifier [[:-]] as a datatype *)
(* constructor for clauses; we even make it infix. *)
(* <abstract syntax and values ((upr))>=        *)
...
fun 'a query database =
  let val builtins = foldl (fn ((n, p), rho) => bind (n, p, rho))
                     emptyEnv (
                          (* Printing a term always succeeds and produces the *)
                         ...
                         (* Two more predicates, [[!]] and [[not]], cannot be *)

                         (* implemented using this technique; they have to be *)

                         (* added directly to the interpreter (Exercises [->] *)

                     (*  and [->]). Here we ensure that they can't be used by *)
...
fun evalcq prompt (t, database : database, echo) =
  case t
    of USE filename => (
                     (* To read from a file, we read without prompting. If we *)
