...
/*
 * To remember all the unit tests in a file, we use a
 * list.
 ...
 * [[UnitTest]]. We use this naming convention
 * throughout our C code. The definitions of our list
 * types are in the lists interface, in chunk [->]. We
 * also define a type for lists of [[Exp]]s.
 */
 ...
 * [[ECHOING]], [[evaldef]] prints the values and names
 * of top-level expressions and functions. As with the
 * formal rules of the operational semantics, we can see
 * from the result types that evaluating a [[Def]] does
 * not produce a [[Value]], but evaluating an [[Exp]]
 * does produce a value. Both kind of evaluations can
 * have side effects on environments.
 */

/*
 * The function [[readevalprint]] consumes all of an
 * input source, evaluating each extended definition,
 * then running all the unit tests. As for [[evaldef]],
 ...
 * which they appear. We use the auxiliary function
 * [[tests_passed]], which also counts the number of
 * tests passed. The auxiliary function
 * [[report_test_results]] prints a report of the
 * results. The reporting code is shared among all
