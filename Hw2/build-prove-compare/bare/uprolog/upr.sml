...
(* abstract syntax and values ((upr)) 627a *)
datatype term = VAR     of string
              | LITERAL of int
              | APPLY   of string * term list
(* abstract syntax and values ((upr)) 627b *)
type goal = string * term list
(* abstract syntax and values ((upr)) 627c *)
datatype clause = :- of goal * goal list
infix 3 :-
(* abstract syntax and values ((upr)) 628 *)
datatype cq
  = ADD_CLAUSE of clause
  | QUERY      of goal list
  | USE of string
(* abstract syntax and values ((upr)) 649a *)
(* string conversions ((upr)) 837 *)
...
(* substitution and unification ((upr)) 650b *)
(* substitution on terms ((prototype)) 650a *)
...
(* substitution and unification ((upr)) 650c *)
...
(* substitution and unification ((upr)) 650d *)
type 'a set = 'a list
val emptyset = []
fun member x = 
...
(* substitution and unification ((upr)) 651a *)
fun freevars t =
...
(* substitution and unification ((upr)) 651b *)
local
  val n = ref 1
in
...
(* substitution and unification ((upr)) 651c *)
fun freshen c =
...
(* substitution and unification ((upr)) 652 *)
(* unification ((prototype)) 651d *)
exception Unify
fun unifyTerm (t1, t2) = 
...
fun lower "is" = RESERVED "is"
  | lower s    = LOWER s
(* lexical analysis ((upr)) 839c *)
fun anonymousVar () =
  case freshVar ""
    of VAR v => UPPER v
     | _ => let exception ThisCan'tHappen in raise ThisCan'tHappen end
(* lexical analysis ((upr)) 839d *)
local
  (* character-classification functions for \uprolog 839a *)
  val symbols = explode "!%^&*-+:=|~<>/?`$\\"
  ...
  (* lexical utility functions for \uprolog 840a *)
  ...
  (* lexical utility functions for \uprolog 840b *)
  ...
  val _ = op unrecognized : char list error -> ('a error * 'a error stream)
                                                                          option
  (* lexical utility functions for \uprolog 840c *)
  fun nextline (file, line) = (file, line+1)
  (* type declarations for consistency checking *)
  val _ = op nextline : srcloc -> srcloc
in
  (* lexical analyzers for for \uprolog 840d *)
  ...
  fun eol chars =
    case streamGet chars
      of SOME (EOL _, chars) => SOME (OK (), chars)
       | _ => NONE
  (* lexical analyzers for for \uprolog 841a *)
  fun charEq c =
    sat (fn c' => c = c') char
  fun manySat p =
    many (sat p char)
  ...
  (* lexical analyzers for for \uprolog 841b *)
  val ordinaryToken =
  ...
  (* lexical analyzers for for \uprolog 841c *)
...
fun prologReader noisy initialMode (name, lines) =
  let val (ps1, ps2) = (mprompt initialMode, "   ")
      ...
      (* utility functions for [[prologReader]] 846b *)
      ...
      (* utility functions for [[prologReader]] 847a *)
      ...
      val _ = op skipPastDot : token located inline stream -> token located
                                                                   inline stream
      (* utility functions for [[prologReader]] 847b *)
        ...
        ; if startsWithEOF tokens then
            ...
            case cq_or_mode mode tokens
              of SOME (OK (CQ cq),         tokens) => SOME (cq, (ps1, mode,
               ...
               | NONE =>
                     (* fail epically with a diagnostic about [[tokens]] 847c *)
...
fun evalcq prompt (t, database : database, echo) =
  case t
    of USE filename => ((* read from file [[filename]] 654a *)
     ...
     | QUERY gs     => ((* query goals [[gs]] against [[database]] 654b *)
                        query database gs
                          (fn theta => fn resume =>
