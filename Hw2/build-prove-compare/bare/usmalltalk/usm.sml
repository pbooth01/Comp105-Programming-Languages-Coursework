...
(* abstract syntax and values ((usm)) 565a *)
datatype exp = VAR     of name
             ...
             | VALUE   of value
             | SUPER
(* abstract syntax and values ((usm)) 565b *)
...
and method_impl = USER_IMPL of name list * name list * exp
                | PRIM_IMPL of name
(* abstract syntax and values ((usm)) 566a *)
and xdef      = DEF  of def
              | USE  of name
              | TEST of unit_test
and unit_test = CHECK_EXPECT of exp * exp
              | CHECK_ERROR  of exp
(* abstract syntax and values ((usm)) 566b *)
and rep = USER     of value ref env (* collection of named instance variables *)
        ...
        | CLASSREP of class
(* abstract syntax and values ((usm)) 566c *)
...
(* abstract syntax and values ((usm)) 566d *)
and method
...
(* abstract syntax and values ((usm)) 574c *)
local 
...
(* definition of [[processXDef]], which can modify [[unitTests]] and call [[errmsg]] 315a *)
            ...
            val _ = op processDef : def  * 'envs -> 'envs
              in  (case xd
                    ...
                    (* more read-eval-print handlers ((usm)) 592b *)
                    | Subscript       => continue
...
(* primitive methods for [[Object]] and [[UndefinedObject]] 577b *)
fun eqRep ((cx, x), (cy, y)) = 
...
(* primitive methods for [[Object]] and [[UndefinedObject]] 578b *)
fun defaultPrint (self as (c, _)) = ( app print ["<", className c, ">"]; self )
(* primitive methods for [[Object]] and [[UndefinedObject]] 578d *)
...
(* primitive methods for [[Object]] and [[UndefinedObject]] 578e *)
...
(* built-in classes [[Object]] and [[UndefinedObject]] 583a *)
val objectClass = 
...
(* built-in classes [[Object]] and [[UndefinedObject]] 583b *)
val nilClass = 
...
(* built-in classes [[Object]] and [[UndefinedObject]] 583c *)
val nilValue = 
...
(* primitive methods for remaining classes 578f *)
...
(* primitive methods for remaining classes 579a *)
...
(* primitive methods for remaining classes 580c *)
...
(* primitive methods for remaining classes 580d *)
...
(* primitive methods for remaining classes 581a *)
...
(* primitive methods for remaining classes 581b *)
local
...
(* primitive methods for remaining classes 582 *)
...
(* primitive methods for remaining classes 617a *)
...
(* remaining built-in classes 583d *)
val classClass =
...
(* tracing ((usm)) 831b *)
local
  (* private state and functions for printing indented traces ((usm)) 831c *)
  fun traceMe xi =
  ...
  (* private state and functions for printing indented traces ((usm)) 831d *)
  val tindent = ref 0
  fun indent 0 = ()
    | indent n = (print "  "; indent (n-1))
  (* private state and functions for printing indented traces ((usm)) 832a *)
...
(* private state and functions for mainting a stack of source-code locations ((usm)) 832b *)
  ...
  fun pop () = case !locationStack
                 of []     => raise InternalError "tracing stack underflows"
                  | h :: t => locationStack := t
in
  (* exposed tracing functions ((usm)) 832c *)
...
fun eval (e, rho, superclass, xi) =
  let (* local helper functions of [[eval]] 584b *)
      ...
      (* local helper functions of [[eval]] 585a *)
      ...
      (* local helper functions of [[eval]] 585b *)
      ...
      val _ = op applyClosure : (name list * exp list * value ref env * class) *
                                                             value list -> value
      (* function [[ev]], the evaluator proper ((usm)) 586a *)
      fun ev (VALUE v) = v
      (* function [[ev]], the evaluator proper ((usm)) 586b *)
        | ev (LITERAL c) = 
            (case c of NUM n => mkInteger n
                     | SYM n => mkSymbol n
                     | _ => raise InternalError "unexpected literal")
      (* function [[ev]], the evaluator proper ((usm)) 586c *)
      ...
      (* function [[ev]], the evaluator proper ((usm)) 586d *)
        | ev (SUPER) = ev (VAR "self")
      (* function [[ev]], the evaluator proper ((usm)) 587a *)
        | ev (BEGIN es) =
            let fun b (e::es, lastval) = b (es, ev e)
                  | b (   [], lastval) = lastval
            in  b (es, nilValue)
            end
      (* function [[ev]], the evaluator proper ((usm)) 587b *)
        | ev (BLOCK (formals, body)) = mkBlock (formals, body, rho, superclass)
      (* function [[ev]], the evaluator proper ((usm)) 588 *)
        | ev (SEND (srcloc, message, receiver, args))  =
            let val obj as (class, rep) = ev receiver
                ...
                val dispatchingClass = case receiver of SUPER => superclass | _
                                                                        => class
                (* definitions of message-tracing procedures 589 *)
...
(* evaluation ((usm)) 590a *)
(* definition of function [[primitiveMethod]] 590c *)
val primitiveMethods = methods ((* primitive methods [[::]] 578a *)
                                primMethod "eqObject" (binaryPrim (mkBoolean o
                                                                      eqRep)) ::
                                (* primitive methods [[::]] 578c *)
                                primMethod "print" (unaryPrim defaultPrint) ::
                                (* primitive methods [[::]] 579c *)
...
fun runTests (tests, xi) =
  let fun ev e = eval (e, emptyEnv, objectClass, xi)
      ...
      (* definition of function [[expString]] ((usm)) 835 *)
      fun expString e =
        let fun bracket s = "(" ^ s ^ ")"
          ...
          of (_, 0) => ()  (* no report *)
           ...
           | _ => if npassed = ntests then
                     ...
                     app print [Int.toString npassed, " of ", Int.toString
                                                                         ntests,
                                " tests passed.\n"]
      (* unit-testing utilities ((usm)) 833c *)
      ...
      (* unit-testing utilities ((usm)) 834a *)
          ...
          of (OK check, OK expect) => 
           ...
           | (_, ERROR _) =>
               fail  [cxfailed, "evaluating ", expString expectx,
                                                            " caused an error."]
      (* unit-testing utilities ((usm)) 834b *)
      ...
      (* unit-testing utilities ((usm)) 834c *)
      fun testsPassed (tests, xi) =
        let (* definition of function [[passes]] ((usm)) 834d *)
