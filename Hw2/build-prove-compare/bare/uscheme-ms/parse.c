...
/* parseexp [[LIST]] and return (BUG: software can't tell where this code came from) */
        {
...
/* parseexp let and put the result in [[rv]] (BUG: software can't tell where this code came from) */
                Letkeyword letword;
                Par letbindings;
 ...
 /* [[RBR else LBR]] possibly parse expressions that are in \uschemeplus 735a */
                } else if (first == strtoname("break")) {
                    /* parseexp [[break]] and put the result in [[rv]] 735b */
                    if (el != NULL)
                        error("%p: usage: (break)", p);
                    rv = mkBreakx();
                } else if (first == strtoname("continue")) {

                   /* parseexp [[continue]] and put the result in [[rv]] 735c */
                    if (el != NULL)
                        error("%p: usage: (continue)", p);
                    rv = mkContinuex();
                } else if (first == strtoname("return")) {
                    /* parseexp [[return]] and put the result in [[rv]] 735d */
                    if (lengthEL(el) != 1)
                        error("%p: usage: (return exp)", p);
                    rv = mkReturnx(nthEL(el, 0));
                } else if (first == strtoname("try-catch")) {

                  /* parseexp [[try-catch]] and put the result in [[rv]] 735e */
                    rv = mkTryCatch(nthEL(el, 0), nthEL(el, 1));
                } else if (first == strtoname("throw")) {
                    /* parseexp [[throw]] and put the result in [[rv]] 735f */
                    if (lengthEL(el) != 1)
...
/* parsesx [[LIST]] and return (BUG: software can't tell where this code came from) */
        if (p->u.list == NULL)
            return mkNil();
        else {
            Value v, w, pair;
