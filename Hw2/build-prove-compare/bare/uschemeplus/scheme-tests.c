...
int tests_passed(UnitTestlist tests, Env rho) {
    if (tests == NULL)
        ...
        case CHECK_EXPECT:
          ...
          /* run [[check-expect]] test [[t]], returning [[n]] or [[n+1]] 713b */
            {   Value check;    /* results of evaluating first expression */
                Value expect;   /* results of evaluating second expression */
                if (setjmp(testjmp)) {

/* report that evaluating [[t->u.check_expect.check]] failed with an error 714b */
                    fprint(stderr,
...
/* report that evaluating [[t->u.check_expect.expect]] failed with an error 714c */
                    fprint(stderr,
                     "Check-expect failed: expected %e to evaluate to the same "
                ...
                if (!equalpairs(check, expect)) {
                    /* report failure because the values are not equal 714a */
                    fprint(stderr,
        ...
        case CHECK_ERROR:
           ...
           /* run [[check-error]] test [[t]], returning [[n]] or [[n+1]] 713c */
       ...
       /* report that evaluating [[t->u.check_error]] produced [[check]] 714d */
                fprint(stderr,
