...
fun typeof (e, gamma, delta) =
 ...
 (* function [[ty]], to check the type of an expression, given $\itenvs$ 336b *)
      fun ty (LITERAL v) = (case v of 
                           NIL => FORALL (["'a"], listtype tyvarA)
                           | NUM _ => inttype
                           | BOOL _ => booltype
                           | SYM _ => symtype
                           | PAIR (a, NIL) => listtype (ty (LITERAL a))
                           | PAIR (b, c) => if eqType( (listtype (ty (LITERAL b))), (ty (LITERAL c))) then listtype  (ty (LITERAL b))
                                            else raise TypeError("Pair contins values of unequal types")
         ...
         | ty (SET (x, e)) = 
                  let val tau_x = ty (VAR x)
                      val tau_e = ty e
                  in if eqType(tau_x, tau_e) then tau_e
                    else raise TypeError("Expression is of a different type than variable")
                  end

         | ty (WHILEX (e1, e2)) =
                  let val tau1 = ty e1
                      val tau2 = ty e2
                  in if eqType(tau1, booltype) then unittype
                    else raise TypeError("First Expression needs to be of type bool, but is of type" ^ typeString tau1)
                  end
         ...
         | ty (IFX (e1, e2, e3)) =
                  let val tau1 = ty e1
                      val tau2 = ty e2
                      val tau3 = ty e3
                  in if eqType(tau1, booltype) 
                        then if eqType(tau2, tau3)
                                then tau2
                             else raise TypeError ("In Expression, true case has type " ^
                                                      typeString tau2 ^ " but false case has" ^
                                                      " a differing type of " ^ typeString tau3) 

                      else raise TypeError ("First Condition in IF expression should have type " ^
                                typeString booltype ^ ", but instead has type " ^
                                typeString tau1)
                  end

         | ty (BEGIN (elist)) =
                  let val (typelist) = (map ty elist) 
                      fun getEnd tau [] = tau
                      | getEnd tau (h::t) = getEnd h t
                  in getEnd unittype typelist
                  end

         | ty (LETX(LET, tupList, e)) =
                  let val (names, types) = (map fst tupList, (map ty (map snd tupList)))
                  in 
                    typeof (e, (bindList(names, types, gamma)), delta)
                  end

         | ty (LETX (LETSTAR, h :: t, e)) = 
                  ty (LETX (LET, [h], LETX(LETSTAR, t, e)))
         | ty (LETX (LETSTAR, _, e)) = ty e

         | ty (LAMBDA lambda_exp) =
                  let val (names, types) = ((map fst (fst (lambda_exp))), (map snd (fst (lambda_exp))))
                  in (funtype (types, typeof((snd (lambda_exp)), (bindList(names, types, gamma)), delta)))
                  end

         | ty (APPLY (e, actuals)) = 
                  let 
                    val tau_e = ty e
                    val acTypes = (map ty actuals)
                  in
                    case tau_e
                      of (CONAPP (TYCON "function", [CONAPP (TYCON "tuple", args), result])) =>
                                  if eqTypes (acTypes, args) then
                                    result
                                  else
                                    raise TypeError ("Parameters dont match expected parameters for function")
                          | _ => raise TypeError("Incorrect function. There is an error in the function syntax")
                  end

         | ty (TYAPPLY (e, tylist)) = instantiate (ty e, tylist, delta)

         | ty (TYLAMBDA (alphas, e)) =
                  let
                    val delta' = foldl (fn (a, delta) => bind (a, TYPE, delta)) delta alphas
                  in
...
fun elabdef (d, gamma, delta) = 
  case d
      of 
      (* cases for elaborating definitions in \timpcore 339e *)
           VAL (x, e) => ((bind (x, typeof(e, gamma, delta), gamma)),
                                                          (typeString (typeof (e, gamma, delta))))
           |VALREC (x, tyex, e) => ((bind (x, tyex, gamma)),
                                                  (typeString (typeof (e, gamma, delta))))
           (* cases for elaborating definitions in \timpcore 340a *)
           | EXP e => elabdef (VAL ("it", e), gamma, delta)

           | DEFINE (name, tyex, lambda_exp) => 
              let
                val (types) = (map snd (fst lambda_exp)) 
              in elabdef (VALREC (name, funtype(types, tyex), 
                                              LAMBDA(lambda_exp)), gamma, delta)
              end
