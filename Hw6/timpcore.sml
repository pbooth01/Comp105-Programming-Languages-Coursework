...
(* function [[ty]], to check the type of an expression, given $\itenvs$ ((prototype)) 345 *)
      | ty (AGET (a, i))       = (case (ty a, ty i) of
                                  (ARRAYTY t, INTTY) => t
                                  | (ARRAYTY _, _) => raise TypeError("Cannot access array index. Variable i should be of type integer") 
                                  | (_, _) => raise TypeError("Cannot access property length of non-array variable"))


      | ty (ASET (a, i, e))    = (case (ty a, ty i, ty e) of
                                  (ARRAYTY t1, INTTY, t2) => if eqType(t1, t2) then
                                                                t1
                                                              else raise TypeError("Cannot set value of mismatched types") 

                                  | (ARRAYTY t1,_,t2) => raise TypeError("Cannot access array index. Variable i should be of type integer")
                                  | (_,INTTY,_) => raise TypeError("Cannot access array index of variable that is not an array")
                                  | (_,_,_) => raise TypeError("Cannot set value of mismatched types. Cannot access array index of variable that is not an array"))


      | ty (AMAKE (len, init)) = (case (ty len, ty init) of
                                  (INTTY, t) => ARRAYTY t
                                  | (_, t) => raise TypeError("Cannot make array od length i. Variable i should be of type integer"))

      | ty (ALEN a)            = (case (ty a) of
                                  (ARRAYTY t) => INTTY
                                  | (_) => raise TypeError("Cannot access property length of non-array variable")) 
